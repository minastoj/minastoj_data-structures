...
protected:
    ...
    bool subtree_is_balanced(Node<Key, Value>* curr) const;
    int height_counter(Node<Key, Value>* curr, bool& unbalanced) const;
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    if(root_ == NULL)
    {
        // create a node for that keyValuePair
    ...
    while(temp != NULL) // while temp is a valid node
    {
        ...
        {
            temp->setValue(keyValuePair.second); // overwrite the current value
            break;
        }
        else if(keyValuePair.first < temp->getKey()) // need to go left
        {
...
{
    Node<Key, Value>* removed_node = internalFind(key);
    if(removed_node == NULL) return;
    
    Node<Key, Value>* parent = removed_node->getParent();
    ...
    {
        Node<Key, Value>* pred = predecessor(removed_node);
...
{
    // done
    if(current->getLeft() != NULL) // there is a left child
    {
        ...
        return current; // return the right most child
    }
    else // there is no left child node --> go up!
    {
...
BinarySearchTree<Key, Value>::successor(Node<Key, Value>* current)
{
    ...
    if(current->getRight() != NULL) // there is a right child
    {
        ...
        return current; // return the left most child
    }
    else // there is no right child node --> go up!
    {
        if (current->getParent() == NULL) {
            return NULL;
        }
...
BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    {
        temp = temp->getLeft(); // set temp to its left child
    }
    return temp; // return left-most child, which is the smallest
}
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
    // done
    Node<Key, Value>* curr = root_;
    while(curr != NULL) // while curr is a valid node
    {
        if(curr->getKey() == key)
        {
            return curr;
        ...
        {
            curr = curr->getRight(); // go to right child
        }
    }
    return NULL;
}
...
bool BinarySearchTree<Key, Value>::isBalanced() const
{
    ...
    height_counter(root_, unbalanced);
    return !unbalanced;
}

template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::height_counter(Node<Key, Value>* curr, bool& unbalanced) const
{
    // if the node is null, return 0
    if(curr == NULL) return 0;

    int left_height = height_counter(curr->getLeft(), unbalanced);
    int right_height = height_counter(curr->getRight(), unbalanced);
