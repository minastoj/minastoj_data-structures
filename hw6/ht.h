...
struct LinearProber : public Prober<KeyType> 
{
    HASH_INDEX_T next() 
    {
        if(this->numProbes_ == this->m_) { // indicates failure to find the key or an empty slot
            return this->npos; 
        }
...
private:
    ...
    size_t valid_items; // keeps track of valid, undeleted items
    size_t num_items_; // keeps track of the total number of items
    double alpha_; // holds alpha
};
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    mIndex_ = 0; // start with capacity of 11
    num_items_ = 0;
    valid_items = 0; 
    alpha_ = resizeAlpha;

    // fill table_ with nullptrs to start
...
{
    if(((double)this->num_items_ / (double)this->table_.size()) >= this->alpha_) // adding item will surpass alpha
    {
        resize();
    }
    HASH_INDEX_T loc = probe(p.first); // find an open spot with the key
    if(loc != npos)
    {
        if(table_[loc] == nullptr) // gave me a free spot
        {
            HashItem* item = new HashItem(p);
            table_[loc] = item;
            num_items_++; // increment to account for an additional item being added
            valid_items++;
        }
        else if(kequal_(table_[loc]->item.first, p.first)) // gave me a location with an item in it
        {
            // update the item value with new value
            table_[loc]->item.second = p.second;
        }
    }
    else
    {
        throw std::logic_error("No space to insert");
    }
}
...
{
    HASH_INDEX_T loc = probe(key); // find the HashItem we want to remove
    if(loc != npos) // key exists in table
    {
        if(table_[loc] != nullptr) // key is in the table
        {
            this->table_[(int)loc]->deleted = true; // make deleted for this item set to true
            valid_items--;
        }
    }
}
...
{
    this->mIndex_++; // get the next capacity in line

    if(this->mIndex_ == sizeof(CAPACITIES)) // there are no more capacities to size up to
    {
        throw std::logic_error("Unable to resize");
    }

    std::vector<HashItem*> new_table_;

    // fill new_table_ with next capacity's worth of nullptrs
    for(int i = 0; i < (int)CAPACITIES[mIndex_]; i++)
    {
        new_table_.push_back(nullptr);
    }

    table_.swap(new_table_);

    this->num_items_ = 0;
    this->valid_items = 0;

    // loop through old table
    for(int j = 0; j < (int)CAPACITIES[mIndex_ - 1]; j++)
    {
        // check that the index isn't a nullptr
        if(new_table_[j] != nullptr)
        {
            // if the current item is set to be deleted, delete it
            if(new_table_[j]->deleted == true)
            {
                delete new_table_[j];
            }
            else // item isn't meant to be deleted
            {
                // rehash it
                HASH_INDEX_T loc = probe(new_table_[j]->item.first);

                // insert into new table
                table_[loc] = new_table_[j];
